/**
 * GeminiLiveProvider — browser-direct WebSocket to the Gemini Live API
 * using an ephemeral token minted by the Xovera app's public voice-demo
 * endpoint. Ported from the VX1 ghl-agent copilot provider (the same
 * class that powers the /try/<slug> prospect demos); trimmed to the
 * types this site needs but behaviorally identical.
 *
...
export interface RealtimeConnectionInfo {
  ...
  token: string;
  /** Vendor model id the token was minted for. */
...
export interface RealtimeProviderConfig {
  ...
  /**
   * Vendor-shaped session config echoed by the server. Passed verbatim to
   * the vendor SDK at connect — it must match the config locked inside the
   * ephemeral token byte-for-byte, so the client never constructs it.
   */
...
export class GeminiLiveProvider {
  ...
  private handleMessage(msg: LiveMessage) {
    ...
    if (msg.goAway) {
      ...
      console.info("[VoiceDemo] goAway received, timeLeft:", msg.goAway.timeLeft);
    }
  }
  ...
  private handleClose() {
    ...
    if (this.resumptionHandle && this.reconnects < MAX_RECONNECTS) {
      ...
      void this.openSession().catch((err) => {
        console.error("[VoiceDemo] reconnect failed:", err);
  ...
  nudge(text: string): void {
    // turnComplete:true forces the model to take a turn now — used to
    // make the agent open with its greeting the moment the call connects.
