...
const INITIAL_INPUTS: GymCalcInputs = {
  monthlyInquiries: INPUT_LIMITS.monthlyInquiries.default,
  membershipPrice: INPUT_LIMITS.membershipPrice.default,
  memberStayMonths: INPUT_LIMITS.memberStayMonths.default,
...
export function GymCalculator() {
  const [stage, setStage] = useState<Stage>(1);
  const [inputs, setInputs] = useState<GymCalcInputs>(INITIAL_INPUTS);
  ...
  const [results, setResults] = useState<GymCalcResults | null>(null);

  // Anti-spam — same contract as the contact form.
  ...
  const updateInput = <K extends keyof GymCalcInputs>(
    key: K,
    value: GymCalcInputs[K],
  ) => {
    markStarted();
  ...
  const validateField = (key: keyof Details): string | undefined => {
    ...
    else if (key === "gymName" && !String(v).trim())
      msg = "Please enter your gym or studio name.";
    else if (key === "email") {
      if (!String(v).trim()) msg = "Please enter your email.";
      else if (!emailLooksValid(String(v)))
  ...
  const validateDetails = (): boolean => {
    const fields: (keyof Details)[] = ["firstName", "gymName", "email", "phone"];
    let ok = true;
        ...
        <div className="rounded-lg bg-card p-6 shadow-card md:p-10">
          ...
          <div className="mt-7 grid grid-cols-1 gap-4 sm:grid-cols-3">
            <ResultStat
              ...
              label="new membership revenue likely missed each month"
              emphasize
            />
            <ResultStat
              value={formatUSD(results.twelveMonthRevenue)}
              label="revenue impact over the next 12 months"
              emphasize
            />
            <ResultStat
              value={`~${results.lostMembersPerMonth}`}
              label="members per month going to nearby gyms instead"
            />
          </div>

          <p className="mt-7 max-w-[58ch] text-[15px] leading-relaxed text-muted-foreground">
            These are estimates built on industry averages — around 23% of
            calls to service businesses go unanswered, and leads contacted
            within 60 seconds are about 8x more likely to qualify. Your real
            numbers might land higher or lower, and we&rsquo;d be happy to walk
            through them with you properly.
          </p>
      ...
      <div className="rounded-lg bg-card p-6 shadow-card md:p-8">
        ...
        {stage === 1 && (
          <div className="space-y-8">
            ...
            >
              A few quick numbers about your gym.
            </h3>
            <Slider
              label="Inquiries per month"
              hint="Calls, form fills, and DMs combined."
              value={inputs.monthlyInquiries}
              limits={INPUT_LIMITS.monthlyInquiries}
              format={(v) =>
                v >= INPUT_LIMITS.monthlyInquiries.max ? `${v}+` : String(v)
              }
            ...
            <Slider
              ...
              limits={INPUT_LIMITS.membershipPrice}
              format={(v) => `$${v}`}
              onChange={(v) => updateInput("membershipPrice", v)}
            />
            <Slider
              label="How long members usually stay"
              value={inputs.memberStayMonths}
              limits={INPUT_LIMITS.memberStayMonths}
              format={(v) => `${v} mo`}
              onChange={(v) => updateInput("memberStayMonths", v)}
            />
          </div>
        )}
        ...
        {stage === 2 && (
          <div className="space-y-7">
            ...
            <ChipGroup<ResponseSpeed>
              label="When a lead fills out a form or sends a DM, how quickly does someone usually reply?"
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              ...
              <Field
                ...
                onChange={(v) => updateDetail("gymName", v)}
                onBlur={() => blur("gymName")}
                error={touched.gymName ? errors.gymName : undefined}
